...
import { X, ExternalLink, Clock, CheckCircle, XCircle, ArrowRightLeft,
         ArrowUpDown, Globe, TrendingUp, Layers } from "lucide-react";
...
function StatusIcon({ status }: { status: HistoryEntry["status"] }) {
  if (status === "confirmed") return <CheckCircle size={14} className="text-green-500" style={{ color: "#40b66b" }}/>;
  if (status === "failed")    return <XCircle     size={14} style={{ color: "#ff4f4f" }}/>;
  return (
...
function TxRow({ e }: { e: HistoryEntry }) {
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3 border-b transition-colors duration-150 hover:opacity-90"
      ...
      <div className="flex-1 min-w-0">
        ...
        {e.amountIn && (
          <div className="text-[12px] mt-0.5 truncate" style={{ color: "var(--text2)" }}>
            {e.type === "bridge"
              ? `${e.amountIn} ${e.symbolIn} → ${e.toChain}`
              : e.type === "send"
              ? `${e.amountIn} ${e.symbolIn} → ${e.toCountry}`
              : e.amountOut
              ? `${e.amountIn} ${e.symbolIn} → ${e.amountOut} ${e.symbolOut}`
              : `${e.amountIn} ${e.symbolIn}`}
          </div>
        )}
          ...
          <div className="flex items-center gap-2">
            {entries.length > 0 && (
              <button onClick={clear}
                className="text-[12px] border-none cursor-pointer px-2 py-1 rounded-lg
          ...
          {entries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 pb-16">
              ...
              <p className="text-[13px] text-center px-8" style={{ color: "var(--text2)" }}>
                Your swap, bridge and send history will appear here
              </p>
            </div>
          ) : (
